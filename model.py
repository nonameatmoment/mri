...
def createLayer(x, szW, trainning,lastLayer):
    ...
    """
    W=tf.compat.v1.get_variable('W',shape=szW,initializer=tf.compat.v1.keras.initializers.glorot_normal())
...
def getLambda():
    ...
    with tf.compat.v1.variable_scope(tf.compat.v1.get_variable_scope(), reuse=tf.compat.v1.AUTO_REUSE):
        lam = tf.compat.v1.get_variable(name='lam1', dtype=tf.compat.v1.float32, initializer=.05)
    return lam
